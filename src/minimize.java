		...
		for (int j = 0; j < n; j++){ // para cada estado de las clases de equivalencia
			for (int i = 0; i < E; i++){ //para cada signo del alfabeto
				if (ClasesEq[j].size() >1 ){ //si el estado tiene más de un elemento. 
					Set<State> distinguibles = distinguidos(Automaton aut1, ClasesEq,j,sigma[i]); //chequeo si se puede distinguir dentro conjunto de estados j de la clase de equivalencia con el elemento i del alfabeto . De ser posible, devuelvo los estados distinguibles. Si no, devuelvo vacío.
...
public static Set<State> distinguidos (Automaton aut1, Set<State>[] ClasesEq, int j, Character s){
